...
describe('ItemService', () => {
  ...
  const ITEM_INSERT_TEST_REQUEST: IItemInsertRequest = {
    ...
    tags: TEST_ITEM_LIST[0].tags.map(tag => tag.id)
  }
  ...
  beforeEach(() => {
    ...
    when(tagService.removeAllFromItem(anything())).thenReturn(Promise.resolve())
  ...
  it('should update an item', async () => {
    ...
    itemService.setModel(instance(itemModel))
    const update = { ...TEST_ITEM_LIST[0] } as any
    update.tags = TEST_ITEM_LIST[0].tags.map(tag => tag.id)
    await expect(itemService.update(update)).resolves.toEqual(TEST_ITEM_LIST[0])
  })
})
