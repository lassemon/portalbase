...
export default class ItemService {
  ...
  constructor() {
    this.itemModel = new ItemModel({
      tableName: 'items',
      connection,
      columns: ['items.*', 'users.id as author_id', 'users.name as author_name'],
  ...
  public async update(itemUpdate: IItemUpdateRequest): Promise<IItem> {
    try {
      ...
      const dbItem = (await this.itemModel.update(this.itemMapper.mapUpdateToQuery(itemUpdate))) as IDBItem
