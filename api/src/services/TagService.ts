...
import { IItemTagInsertQuery, ITagInsertRequest, ITagsForItemQuery, ITagUpdateRequest } from 'interfaces/requests'
...
export default class TagService {
  ...
  public async update(tagUpdate: ITagUpdateRequest): Promise<ITag> {
    try {
      const tag = (await this.tagModel.update(this.tagMapper.mapUpdateToQuery(tagUpdate))) as IDBTag
      return this.tagMapper.serialize(tag)
    } catch (error) {
      log.error(error)
