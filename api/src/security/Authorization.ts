...
export default class Authorization {
  ...
  public createAuthToken = (user: IUser): string => {
    ...
    log.debug(
      'CREATED AUTH TOKEN THAT IS ISSUED AT ' + moment.unix(this.decodeToken(authToken).iat).format('dd HH:mm:ss')
    )
    log.debug(
      'CREATED AUTH TOKEN THAT EXPIRES IN ' + moment.unix(this.decodeToken(authToken).exp).format('dd HH:mm:ss')
    )
  ...
  public createRefreshToken = (user: IUser): string => {
    ...
    log.debug(
      'CREATED REFRESH TOKEN THAT IS ISSUED AT ' + moment.unix(this.decodeToken(refreshToken).iat).format('dd HH:mm:ss')
    )
    log.debug(
      'CREATED REFRESH TOKEN THAT EXPIRES IN ' + moment.unix(this.decodeToken(refreshToken).exp).format('dd HH:mm:ss')
    )
